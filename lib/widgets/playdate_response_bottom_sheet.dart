...
class _PlaydateResponseBottomSheetState extends State<PlaydateResponseBottomSheet>
  ...
  Future<void> _loadUserDogs() async {
    try {
      ...
      final dogs = dogData.map<Dog>((data) => Dog(
            ...
            AppCard(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildDetailRow(Icons.schedule, _formatDateTime(_suggestedTime)),
                  _buildDetailRow(Icons.location_on, playdate?['location'] ?? 'Location TBD'),
  ...
  Widget _buildResponseButton(String text, IconData icon, Color color, VoidCallback onPressed) {
      ...
      children: [
        ...
        if (_availableDogs.isEmpty)
          ...
          Wrap(
            spacing: 8,
            runSpacing: 8,
            children: _availableDogs.map((dog) => _buildDogSelectionChip(dog)).toList(),
          ),
      ],
    );
  }
    ...
    return ActionChip(
      ...
      onPressed: () async {
        final date = await showDatePicker(
          context: context,
          initialDate: _suggestedTime ?? DateTime.now().add(const Duration(days: 1)),
        ...
        if (date != null && mounted) {
          final time = await showTimePicker(
            context: context,
            initialTime: TimeOfDay.fromDateTime(_suggestedTime ?? DateTime.now()),
          );
          
          if (time != null) {
            setState(() {
              _suggestedTime = DateTime(date.year, date.month, date.day, time.hour, time.minute);
            });
          }
        }
      },
  ...
  Future<void> _handleCounterProposal() async {
    ...
    try {
      ...
      final success = await PlaydateRequestService.respondToPlaydateRequest(
        ...
        message: _message.isNotEmpty ? _message : 'I have some suggestions for the playdate!',
