...
class PlaydateActionButton extends StatefulWidget {
	...
	const PlaydateActionButton({
		...
		this.padding = const EdgeInsets.symmetric(horizontal: 12, vertical: 8),
					...
					playdate != null || (targetUserId != null && targetDogId != null && myDogId != null),
...
class _PlaydateActionButtonState extends State<PlaydateActionButton> {
	bool _loading = true;
	String _state = 'none'; // none | pending_outgoing | pending_incoming | confirmed
	...
	void initState() {
		...
		Future<void> _refresh() async {
			...
			if (user == null) {
				setState(() { _state = 'none'; _loading = false; });
				return;
			}
			...
			try {
				...
				final confirmed = await SupabaseConfig.client
						.from('playdates')
						.select('id, title, location, scheduled_at, status, organizer_id, participant_id')
				...
				for (final p in confirmed) {
					final between = (p['organizer_id'] == myId && p['participant_id'] == targetUserId) ||
							(p['organizer_id'] == targetUserId && p['participant_id'] == myId);
					if (between && p['status'] == 'confirmed') { foundConfirmed = p; break; }
				}
				if (foundConfirmed != null) {
					setState(() { _state = 'confirmed'; _playdate = foundConfirmed; _loading = false; });
					return;
				}
				...
				if (outgoing.isNotEmpty) {
					setState(() { _state = 'pending_outgoing'; _requestId = outgoing.first['id']; _loading = false; });
					return;
				}
				...
				if (incoming.isNotEmpty) {
					setState(() { _state = 'pending_incoming'; _requestId = incoming.first['id']; _loading = false; });
					return;
				}

				setState(() { _state = 'none'; _loading = false; });
			} catch (_) {
				if (mounted) setState(() { _state = 'none'; _loading = false; });
			}
		}
			...
			builder: (ctx) => SafeArea(
				child: Padding(
					...
					child: Column(
						...
						children: [
							Text('Playdate Scheduled', style: Theme.of(ctx).textTheme.titleMedium),
							const SizedBox(height: 8),
							Text('${_playdate!['location'] ?? 'Location TBD'} • ${_playdate!['scheduled_at'].toString()}'),
											...
											onPressed: () async {
												...
												final ok = await showDialog<bool>(
													...
													builder: (_) => AlertDialog(
														...
														content: const Text('Other participant(s) will be notified.'),
														actions: [
												...
												if (ok == true) {
													await PlaydateManagementService.cancelPlaydate(
														...
														cancelledByUserId: SupabaseConfig.auth.currentUser!.id,
													);
													_refresh();
												}
											},
	...
	Future<void> _rescheduleDialog() async {
		...
		final locationCtrl = TextEditingController(text: _playdate!['location'] ?? '');
		showDialog(
			context: context,
					...
					title: const Text('Reschedule'),
						content: SingleChildScrollView(
							child: Column(
								...
								children: [
									ListTile(
										title: Text(pickedDate == null ? 'Pick date' : '${pickedDate!.year}-${pickedDate!.month}-${pickedDate!.day}'),
									...
									ListTile(
										title: Text(pickedTime == null ? 'Pick time' : pickedTime!.format(ctx)),
								...
								onPressed: () async {
									if (pickedDate != null && pickedTime != null) {
										...
										await PlaydateManagementService.reschedulePlaydate(
											...
											newLocation: locationCtrl.text.isEmpty ? null : locationCtrl.text,
										);
	...
	Widget build(BuildContext context) {
		if (_loading) {
			return Padding(
				...
				child: const SizedBox(width: 42, height: 42, child: CircularProgressIndicator(strokeWidth: 2)),
			);
		}
