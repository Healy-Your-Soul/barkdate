...
Future<void> testDatabaseFunctions() async {
  ...
  try {
    ...
    final tablesResult =
        await supabase.from('dog_owners').select('id').limit(1);
    print('✅ dog_owners table exists');

    final shareLinksResult =
        await supabase.from('dog_share_links').select('id').limit(1);
    print('✅ dog_share_links table exists');

    print('\n2️⃣ Testing helper functions exist...');

    // Test 2: Check if functions exist
    final functionsTest = await supabase.rpc('get_user_accessible_dogs',
